...
const addMetadata = async (chatId, bot, metadataRes, metadataDetails, metadataFilePath) => {
  try {
    ...
    // if (confirmationResponse === "confirm") {
      ...
      // Send a message indicating that metadata is added
    bot.sendMessage(
      chatId,
      `📖 Metadata added successfully! ✅\n\n🟢 Name: ${metadataInfo.name}\n🟢 Symbol: ${metadataInfo.symbol}\n🔗 URI: ${metadataInfo.uri}`
    );
...
//  addMetadataModule.addMetadata(chatId, bot, metadataRes, metadataDetails, metadataFilePath);
    // After metadata processing and saving, call createMintAccountAndMintTokens
  //  await createMintAccountAndMintTokens(chatId, decimal, tokenSupply, bot);
  } catch (error) {
    console.error(error);
  }
};

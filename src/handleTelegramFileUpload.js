...
const saveMetadataToFile = (metadataDetails, numberedFileName, directoryPath) => {
  try {
...
const pinFileToIPFS = async (filePath, chatId, bot) => {
  try {
    ...
    metadataDetails.image = `https://gateway.pinata.cloud/ipfs/${res.data.IpfsHash}/${numberedFileName}`;
    ...
    bot.once("document", async (msg) => {
      try {
        ...
        fileStream.on("finish", async () => {
          ...
          const destinationPath = path.join(pathToFolders.loomDeployedTokenLogo, `${msg.document.file_name}`);
